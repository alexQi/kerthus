  ...
  export default defineComponent({
    ...
    setup(props, { attrs, slots, emit, expose }) {
      ...
      const getBindValues = computed(() => {
        let propsData = {
          ...
          onCheck: (v: CheckKeys, e) => {
            const checkedValue = toRaw(state.checkedKeys);
            let currentValue = isArray(checkedValue) ? checkedValue : checkedValue.checked;
            if (searchState.startSearch) {
              const value = e.node.eventKey;
              currentValue = difference(
                currentValue,
                state.checkStrictly ? [value] : getChildrenKeys(value),
              );
              if (e.checked) {
      ...
      function onStrictlyChange(strictly: boolean) {
        if (props.allowCheckStrictlyChange) state.checkStrictly = strictly;
      }
        ...
        return (
          <div class={[bem(), 'h-full', attrs.class]}>
            {showTitle && (
              <TreeHeader
                checkable={checkable}
                allowCheckStrictlyChange={props.allowCheckStrictlyChange}
                checkAll={checkAll}
                expandAll={expandAll}
                title={title}
                search={search}

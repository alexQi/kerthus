...
const transform: AxiosTransform = {
  ...
  transformResponseHook: (res: AxiosResponse<Result>, options: RequestOptions) => {
    ...
    switch (code) {
      case ResultEnum.TIMEOUT:
        handleUnauthorized(res.config, msg, options.errorMessageMode);
        throw new Error(msg || t('sys.api.errMsg401'));
      default:
        if (msg) {
          timeoutMsg = msg;
        }
  ...
  responseInterceptorsCatch: (axiosInstance: AxiosResponse, error: any) => {
    ...
    const checked = checkStatus(config, error?.response?.status, msg, errorMessageMode);
    if (!checked) {
      return Promise.reject(error);
    }

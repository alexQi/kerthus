      ...
      <template #bodyCell="{ column, record }">
        <template v-if="column.key === 'name'">
          <div class="provider-name">
            <Badge :color="record.default ? '#1677ff' : '#bfbfbf'" />
            <b>{{ record.name || record.provider }}</b>
          </div>
          <div class="muted">{{ record.code || '未设置标识' }}</div>
        </template>
        <template v-else-if="column.key === 'provider'">
          {{ providerLabel(record.provider) }}
        </template>
        <template v-else-if="column.key === 'endpoint'">
          <span class="endpoint">{{ record.endpoint || '未设置' }}</span>
        </template>
...
<script lang="ts">
  ...
  import { Badge, Button, Tag, message } from 'ant-design-vue';
  ...
  export default defineComponent({
    ...
    components: { BasicTable, TableAction, Badge, Button, Tag, Icon, EditForm },
    setup() {

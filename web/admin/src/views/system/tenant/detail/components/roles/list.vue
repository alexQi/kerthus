<template>
  <div class="m-4 mr-0 bg-white">
    <BasicTable @register="registerTable">
      <template #toolbar>
        <a-button v-auth="['system:tenant:detail:role:save']" type="primary" @click="handleCreate">
          新增
        </a-button>
      </template>
            ...
            :actions="[
              {
                ...
                ifShow: hasPermission(['system:tenant:detail:role:save']),
              },
              {
                icon: 'ant-design:delete-outlined',
                color: 'error',
                ifShow: hasPermission(['system:tenant:detail:role:delete']),
                popConfirm: {
                  title: '是否确认删除',
                  placement: 'left',
    ...
    <Form @register="registerModal" @success="handleSuccess" :fetchParams="fetchParams" />
  </div>
</template>
<script lang="ts">
      ...
      const [registerTable, { reload }] = useTable({
        ...
        actionColumn: {
          ...
          ifShow: hasPermission([
            'system:tenant:detail:role:save',
            'system:tenant:detail:role:delete',
          ]),
        },
      });

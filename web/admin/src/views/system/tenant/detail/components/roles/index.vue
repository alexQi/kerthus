<template>
  <div class="flex">
    <List class="w-4/11" @select="handleSelectRole" :fetchParams="fetchParams" />
    <div class="w-7/11 m-4 mr-0 bg-white">
      <Card :tabList="tabList" :activeTabKey="activeTab" @tab-change="handleTabChange">
        <template #tabBarExtraContent>
          <a-button
            v-auth="['system:tenant:detail:role:authRoleResource']"
            :disabled="
              activeTab === 'user' ||
              !Number(roleId) ||
              resourcesLoading ||
        ...
        />
        <Users v-if="activeTab === 'user'" :roleId="roleId" :fetchParams="fetchParams" />
      </Card>
    </div>
  </div>
</template>
<script lang="ts">
  ...
  export default defineComponent({
    name: 'RoleResource',
    components: { List, Resources, Users, Card },
    props: {
      fetchParams: {

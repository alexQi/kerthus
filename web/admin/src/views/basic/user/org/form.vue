<template>
  <div class="m-4 mr-0 bg-white">
    <Card :title="titleRef">
      <template v-if="accessEdit" #extra>
  ...
  export default defineComponent({
    ...
    setup(props, { emit }) {
      ...
      async function fetchInfo() {
        ...
        try {
          const data = await getInfo({ id: props.orgId });
      ...
      async function handleSubmit() {
        ...
        try {
          ...
          const data = await saveItem(values);
          if (data) {
      ...
      return { titleRef, registerForm, resetForm, saving, loading, handleSubmit };
    },
  });
</script>

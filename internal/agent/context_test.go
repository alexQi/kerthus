...
func TestNormalizePageContextReplacesIdentityAndRedactsSecrets(t *testing.T) {
	out, err := NormalizePageContext(PageContext{
		Version:  "old",
		TenantID: 999,
		Filters:  map[string]any{"name": "张三", "password": "do-not-send", "nested": map[string]any{"token": "x", "ok": "yes"}},
	...
	if strings.Contains(string(b), "do-not-send") || strings.Contains(string(b), "token") {

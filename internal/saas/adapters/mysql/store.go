...
var columns = map[ports.Table]string{
	ports.Users: "id phone email name status platform_admin created_at updated_at", ports.Tenants: "id name status verify_status bootstrap_version created_at updated_at expires_at agent_provider agent_model agent_endpoint agent_api_key agent_enabled agent_providers", ports.Members: "id tenant_id user_id status is_default default_app_id default_org_id default_unit_id created_at updated_at",
